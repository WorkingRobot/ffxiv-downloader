use libc::{ENOSYS, EOPNOTSUPP, POSIX_FADV_RANDOM, posix_fadvise64};
...
impl AsyncFile {
    ...
    fn setup(&self) -> io::Result<()> {
        ...
        let result = unsafe { posix_fadvise64(fd, 0, 0, POSIX_FADV_RANDOM) };
        match result {
            ENOSYS | EOPNOTSUPP => {
    ...
    pub async fn pread(&self, offset: usize, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let bytes_read = task::spawn_blocking(move || {
            ...
            // - libc::pread64 is expected to handle raw pointers correctly
            let result =
                unsafe { libc::pread64(fd, buf_ptr as *mut libc::c_void, buf_len, offset) };
            if result < 0 {
    ...
    pub async fn pwrite(&self, offset: usize, data: &[u8]) -> io::Result<usize> {
        ...
        // 4. The pointer is immediately cast back to the correct type in the unsafe block
        // 5. The libc::pwrite64 call will not outlive the data's lifetime
        ...
        task::spawn_blocking(move || {
            ...
            // - We're not dereferencing through Rust, only passing to libc
            // - libc::pwrite64 is expected to handle raw pointers correctly
            // - The data is read-only, so no race conditions with mutations
            let result =
                unsafe { libc::pwrite64(fd, data_ptr as *const libc::c_void, data_len, offset) };
            if result < 0 {
                Err(io::Error::last_os_error())
            } else {
    ...
    pub async fn truncate(&self, size: usize) -> io::Result<()> {
        ...
        if unsafe { libc::ftruncate64(fd, size) } < 0 {
            Err(io::Error::last_os_error())
        } else {
            Ok(())
        }
